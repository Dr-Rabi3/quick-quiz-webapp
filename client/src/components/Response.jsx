...
export default function Response() {
  ...
  const { data: studentData } = useHttp(
    // `https://quickquiz-backend.onrender.com/exam/response/${params.uid}`,
    `http://localhost:3000/exam/response/${params.uid}`,
    requestConfig
  );
  const { data: questions } = useHttp(
    // `https://quickquiz-backend.onrender.com/users/exams:${getExamId()}/questions`,
    `http://localhost:3000/users/exams:${getExamId()}/questions`,
    requestConfig
  );
    const {} = useHttp(
      // `https://quickquiz-backend.onrender.com/notification/:${getExamId()}/:0`,

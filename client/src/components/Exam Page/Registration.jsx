...
export default function Registration({ userId, examId, onStart }) {
  const { data: ExamData } = useHttp(
    // `https://quickquiz-backend.onrender.com/user/exam:${examId}`,
    `http://localhost:3000/user/exam:${examId}`,
    requestConfig
  );
  const { data: userData } = useHttp(
    // `https://quickquiz-backend.onrender.com/user:${userId}/exam`,
    `http://localhost:3000/user:${userId}/exam`,
    requestConfig
  );
